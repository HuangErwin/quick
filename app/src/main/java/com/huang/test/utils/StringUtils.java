...
public class StringUtils {
     ...
     * @return ASCII码
     */
    private int Chn2ASCII(String s) {
        int ascii = 0;
        try {
     ...
     * @return 拼音
     */
    private String Chn2PY(String s) {
        String ret = null;
        int ascii = Chn2ASCII(s);
     ...
     * @return 拼音
     */
    public String getPYFirstLetter(String s) {
        if (isSpace(s)) return "";
        String first, py;
    ...
    public static String getDistance(long juli) {
        if (juli == 0) {
            return "";
        }
        if (juli < 1000) {
    ...
    public static int getDay( String rentStartTime, String rentEndTime) {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        try {
            long l1 = sdf.parse(rentStartTime).getTime() / 1000;
            long l = sdf.parse(rentEndTime).getTime() / 1000;

            long abs = Math.abs(l1 - l);
            int l2 = (int) (abs / (24 * 60 * 60));
            return l2;
...
//        }
//        try {
//            String s = Base64.getEncoder().encodeToString(text.getBytes(charsetName));
//            msgDigest.update(s.getBytes()); // 注意是按照指定编码形式签名
//        } catch (UnsupportedEncodingException e) {
